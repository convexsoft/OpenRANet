...
def data_progress():
    cvxcnn_data = list()
    cvxcnn_target = list()
    with open("../data_generation/data2.csv", "r") as csvfile:
    ...
    train_loader = Data.DataLoader(dataset=train_data,batch_size=2,shuffle=True,num_workers=0)
    ...
    idx = 2
    print("idx:",idx)
    single_data = np.array([x_test[idx].tolist()])
    single_target = y_test[idx]
...
class MLPPregression(nn.Module):
    def __init__(self):
        ...
        self.hidden1 = nn.Linear(in_features=16,out_features=200,bias=True)
        self.hidden2 = nn.Linear(200,200)
        self.hidden3 = nn.Linear(200,100)
        self.predict = nn.Linear(100,4)
    def forward(self,x):
...
def training_progress(train_loader, train_xt, train_yt, test_xt, test_yt,y_test,single_data,single_target):
    ...
    optimizer = SGD(cvxcnnreg.parameters(),lr=0.0004,weight_decay=0.0001)
